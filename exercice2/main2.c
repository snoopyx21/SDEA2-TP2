// NOTE: main a change car il contenait deux bugs. 

// 1) Le prenom jean n'est pas ajouté dans le main car le fichier originel contient
// en fait deux fois jean: jean (1) et jean (2)
// 2) il manquait fclose()
...
int main(int argc, char* argv[argc-1])
{
  ...
  /* init the table. */
  stringtable2 tnames = newStringtable();
  ...
  for (i = 0; i < NN; i++)
  {
	  ...
	  if (idn!=0 && idn != id[i]) printf("warning %s has new identifier: %u versus %u\n", n[i], idn, id[i]);
	  //else
	  {
